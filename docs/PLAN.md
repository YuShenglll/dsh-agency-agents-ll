...
| D7 | 默认提示词语言 | **英文** |
| D8 | 中文质量门禁 | 简介必填，字数与句数入区间；全文译文（若提供）另过结构镜像门禁 |
| D9 | 客户端 UI | 名册浏览、搜索与筛选、启用停用、查看与复制提示词、自定义专家编辑器、输入框 `@` 触发。设置项名为「专家库」，输入栏按钮显示「专家」 |
| D10 | 头像 | 卡片头像是**按专家的 SVG 素材**（`assets/avatar/` 279 张，内联进客户端包为 data URI，见 `docs/AVATARS.md`）。**没有素材的专家回退 frontmatter 里的 emoji** —— 兜底是设计的一部分，上游新增专家不会出现空白 |
| D10b | 头像素材的授权 | AI 生成、项目自有的原创素材，采用 Apache-2.0，记在 `NOTICE` 第 3 条。**未使用任何第三方图标集** |
...
| D16 | 布局不得依赖容器宽度 | 设置面板的滚动条随名册长短出现/消失，**内容框宽度差 8px**（实占，非覆盖式）。所以先**锁死宽度**（`min-height:calc(100% + 1px)` 让滚动条常驻 + `scrollbar-gutter:stable` 预留 gutter），再谈对齐。**不允许**用「逐个元素躲开」的方式治（那治的是症状） |
...
`intro` 是本项目的**主要交付物**：3–8 句中文（实测 279 份落在 3–6 句），讲清这个专家是什么角色、擅长什么、什么时候该找他。
...
**中文名必须全局唯一**（`name-uniqueness` 门禁，硬失败）。这不是审美要求：运行时要按名字解析专家（`resolve_expert`、`describe_expert`、`@` 触发），两个专家同名会让按名召唤变成歧义错误、让两张卡的开关被永久禁用。`slug` 才是唯一键，名字不是——所以这条不变量必须由门禁守住，而不能指望写内容的人记得。
...
`sync/checks.mjs` 跑下列检查（部分是逐文件的、部分是整个名册级的），输出三堆清单。

| 检查 | 抓什么问题 | 硬失败 |
|---|---|---|
| slug 集合一致性 | 英文树与中文树的分区、slug 集合差异 | 否 |
| 中文名全局唯一 | 两个档案共用同一个中文名或英文名——运行时按名字解析会歧义，`@` 引用也会生成解析不了的名字 | **是** |
| manifest 覆盖完整 | 名册条目在 `sync/manifest.json` 里没有记录 | **是** |
| frontmatter 合法性 | 中文档案缺 `name` / `description` / `intro` / `emoji`，引号或围栏未闭合 | **是** |
| `intro` 字数与句数区间 | 简介缺失、过短（像占位）或过长（像贴了一整篇）；字数与句数各一条断言 | **是** |
...
| 长度比区间 | 正文中英字符比落在 `[0.7, 3.2]` 之外（区间由 `pnpm sync:calibrate` 按门禁**同一个度量**标定；当前只有 6 对带正文的样本，见 `checks.mjs` 注释） | 否 |
...
| 风险 | 处置 |
|---|---|
| 中文与英文内容不对等（漏译/臆造） | 第 6 节的段落对齐与长度比检查，机器可查；简介另过字数、句数与必填校验 |
...
| 同一根因反复以不同症状出现 | D16 已经踩过三轮：先治「会伸缩的字段」，再治「右锚定的按钮」，两次都是症状；直到锁死容器内容框宽度才收敛。**诊断时先问「什么东西的尺寸在变」，而不是「哪个元素在动」** |
| 用户态真源与模型侧入口分裂 | 2026 评审的 blocker：浏览器侧已经支持自建专家与启用停用，而 4 个工具仍只读磁盘名册——于是「新建一个专家」在 UI 里全程可行，**唯独最后一步召唤必然失败**，而 63 条测试没有一条覆盖这条主路径。处置：D17/D18 + 一条「自建专家 → summon → 命中 persona」的端到端断言。**教训：一条用户可见的主路径，如果没有一条断言真的走完它，那它就没有被测过** |
| 门禁自己报的状态与自己的判定脱钩 | 2026 评审的 H5/H6：`slug-sets` 的 `status` 是字面量 `'pass'`、`english-byte-identity` 的 `status` 不含它自己产生的硬失败，于是同一次运行可以既打 `PASS` 又列 `HARD FAILURES`。处置：status 一律由该检查自己的 notes/drift 推导。**教训：机器可读的那一份报告才是被消费的那一份，它撒谎比退出码撒谎更隐蔽** |
