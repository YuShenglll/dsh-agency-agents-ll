...
| P4 客户端 + Remote | ✅ | 30 项 verify；`--dump-config` 见主行与 remote 行；**用户在浏览器验收「基本功能实现」**；2026-09-13 修掉界面锁死与白屏（见 4.1），随后三轮界面优化（见 4.2 / 4.3 / 4.4），补 16 项 jsdom 组件测试，合计 **53 项 vitest** |
| P5 发布 | ⏳ | 只差打 tag |
