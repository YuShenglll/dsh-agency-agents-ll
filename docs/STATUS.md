...
## 4.8 页头按钮二次修正：不再锚定右边缘（2026-09-13）

4.7 那条 `:has(> .aall-section){scrollbar-gutter:stable}` **经用户实测无效**，按钮照样跳。

失败原因无法在本机验证（需要看真实 DOM），但结论很清楚：**这个修法把正确性押在了宿主内部结构上**（滚动容器是不是 `.options`、中间有没有多一层包装、`scrollbar-gutter` 与宿主自定义 `::-webkit-scrollbar` 是否共存），而这些我一样都验证不了。`guarded()` 在源码里看起来确实只返回 `<SlotErrorBoundary><RootEntry/></SlotErrorBoundary>`（类组件，不产生 DOM），但源码不等于运行时。

**改法：不再依赖任何宿主细节。** 让按钮**不锚定右边缘**：

```css
.aall-head-text{flex:0 1 auto}   /* 原来是 flex:1 1 260px，会生长 */
.aall-actions{...}                /* 删掉 margin-left:auto */
```

页头因此完全从左往右排布。容器宽度变化 8px 时，**左锚定的内容一个像素都不动** —— 这条不依赖宿主、不依赖滚动条、不依赖 `scrollbar-gutter` 的支持情况，**在数学上不可能失败**。

同时删掉那条 `:has()` 规则：修法既然换了，留着一条已验证无效、且伸进宿主内部的规则没有意义。样式表恢复成**每一条选择器都以 `.aall-` 开头**，并把这条不变量写进了测试。

**仍然存在但用户未报告**：卡片宽度会随滚动条来回变 8px，简介的折行位置随之变化。这是同一根因的另一面，但它不是"按钮跳动"，需要用户确认是否在意。
