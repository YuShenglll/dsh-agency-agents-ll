...
> **另一件悬着的小事**：卡片宽度会随设置面板滚动条的出现/消失来回变 8px，简介折行位置跟着变。已在 5.8 记录，用户尚未表态是否在意。
...
| P4 客户端 + Remote | ✅ | 用户已在浏览器验收；2026-09-13 修掉界面锁死与白屏（5.1），随后七轮界面优化与修复（5.2–5.8） |
| P5 发布 | ⏳ | 只差打 tag。建议在真实召唤一次专家之后再打 |
| P6 头像 | ⏳ | **等用户交付素材**；规格与清单已就绪（见 §1 与 `AVATARS.md`） |

**门禁现状（2026-09-13，HEAD `3fc8095`）**：

```
pnpm build   exit=0
pnpm test    57 passed   （remote 13 + host 24 + client/jsdom 20）
...
| 启用停用（2026-09-13 修复后） | 由 20 条 jsdom 组件测试覆盖，其中 3 条在修复前必失败；**仍需一次真机复测** |
| `.aall-switch` 绝对定位输入的包含块 | 已加 `position:relative` 兜底；未确认宿主对 `input[type=checkbox]` 是否有更高优先级的全局规则 |
| 卡片宽度随滚动条变 8px | **已确认存在、未修**（见 5.8）。不是"按钮跳动"，需要用户确认是否在意 |
...
**这次修正还不够。** 用户随后反馈：页头的按钮应该和下面卡片的右边缘对齐，而且**卡片自己也在跳**（宽度随滚动条来回变 8px）。也就是说 4.6/4.7 治的都是症状，**真正的病根是内容框宽度会变**，只要这个不变，按钮怎么锚都不会跳。

## 5.9 病根：让滚动条永远不消失（2026-09-13）

**为什么前两次都没治好**：4.6 治了「会伸缩的搜索框」，4.7/4.8 治了「锚定右边的按钮」—— **两个都是症状**。任何占满宽度的东西（卡片、简介折行）都会跟着 8px 抖，而卡片占满宽度是改不掉的。
...
**仍未验证**：两层的实际效果需要真机确认。若仍然跳动，下一手是**把内容列宽写死**（用户已授权），或者**让 section 自己成为滚动容器**（`height:100%` + `overflow-y:auto` + 自己的 `scrollbar-gutter:stable`，两层宽度都锁死，代价是嵌套滚动）。
...
pnpm exec vitest run    # 57 项：remote 13 + host 24 + 客户端 jsdom 20
...
| 项 | 说明 |
|---|---|
| **279 张头像素材** | **唯一在等的事**，见 §1 与 5.9。规格与清单已就绪 |
| 卡片宽度随滚动条变 8px | 已知未修，见 5.8。需要用户先表态是否在意 |
...
- 改浏览器端之后 → 必须跑 `pnpm exec vitest run src/client/index.test.ts`：这 **20 条**在 jsdom 里用**真实的 279 份资产**渲染真实组件，是唯一能在没有浏览器的情况下抓到「一次写入锁死整页」「连点被吞」「抛错变白屏」「滚动条一来自适应布局就跑偏」的地方。**新写这类断言时先在旧代码上跑一遍确认它会失败**，否则它只是装饰
- 改资产或术语 → 动 `assets/`、`sync/glossary.json` 后必须跑 `pnpm sync:stamp && pnpm check`
- 改契约 → 先改 `docs/PLAN.md` 再改代码
- **UI 布局铁律**（踩过两次）：**不要让任何东西依赖容器宽度**。设置面板的滚动条会随名册长短出现和消失，内容宽度因此变化 8px；`flex-grow` 的元素会变宽，`margin-left:auto` 的元素会位移。页头与筛选行现在**全部左锚定 + 固定 `flex-basis`**，样式表里**每一条选择器都以 `.aall-` 开头**（有一条测试钉住这一点，别再加 `:has()` 之类的宿主耦合规则）
